...
def show_search():
    ...
    st.header('''Trouvez facilement un film en utilisant le menu déroulant ou en tapant un titre dans la barre de recherche pour vérifier s’il figure dans notre base de données.
                    Une fois votre film sélectionné, nous vous proposons une liste de films similaires.
                    Sous chaque affiche, vous pouvez cliquer sur un bouton pour obtenir plus d’informations détaillées sur le film : synopsis, casting, durée et plus encore.
                    Votre prochaine soirée cinéma commence ici !''')
        ...
        for j, film in enumerate(films_subset.itertuples()):
            with cols[j]:
                ...
                **📅 Release Date:** {film.release_date}
                """)
                if st.button(f'More details - {film.original_title}', key = f'{film.tmdb_id}'):
                    st.session_state.film_id = film.tmdb_id
                    film_url = f"/?page=Details&film_id={int(film.tmdb_id)}"
                    st.markdown(f"[More details - {film.original_title}]({film_url})")
                    #st.query_params["page"] = "Details"

    # cols = st.columns(col)
    # for n,i in enumerate(df.index.to_list()):
    #     image = films.loc[i,'poster_path']
    #     name = films.loc[i,'original_title']
    #     rating = int(films.loc[i,"imdb_averageRating"])
    #     release_date = films.loc[i,"release_date"]
    #     with cols[n]:
    #         st.image(image)
    #         st.markdown(f"""
    #         **🎬 Nom du film :** {name}  
